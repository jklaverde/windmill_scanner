...
from http import simulation as sim, ws_registry
...
class WindmillCreate(BaseModel):
    ...
    @field_validator("sensor_beat_unit", "location_beat_unit")
    @classmethod
    def validate_beat_unit(cls, v: str) -> str:
        if v not in ("ss", "mm", "hh", "dd"):
            raise ValueError("beat unit must be one of: ss, mm, hh, dd")
        return v

    @field_validator("lat_dir")
    @classmethod
    def validate_lat_dir(cls, v: str) -> str:
        if v not in ("N", "S"):
            raise ValueError("lat_dir must be N or S")
        return v

    @field_validator("lon_dir")
    @classmethod
    def validate_lon_dir(cls, v: str) -> str:
        if v not in ("E", "W"):
            raise ValueError("lon_dir must be E or W")
        return v
...
@router.get(
    "/farms/{farm_id}/windmills",
    summary="List windmills in a farm",
    description="Return all windmills for a farm, sorted alphabetically by name.",
)
def list_windmills(farm_id: int, db: Session = Depends(get_db)):
    """Return windmill list for the given farm."""
    farm = farm_repo.get_by_id(db, farm_id)
    if not farm:
        raise HTTPException(status_code=404, detail="Farm not found.")
    wms = windmill_repo.get_by_farm(db, farm_id)
    return [windmill_repo.windmill_to_dict(w) for w in wms]
...
async def delete_windmill(windmill_id: str, db: Session = Depends(get_db)):
    """Delete a windmill. 409 if it is running. Sends type:error to open WS connections."""
    wm = windmill_repo.get_by_windmill_id(db, windmill_id)
    if not wm:
        raise HTTPException(status_code=404, detail="Windmill not found.")
    if wm.is_running:
        raise HTTPException(
            status_code=409,
            detail="Stop the windmill data stream before deleting it.",
        )
...
async def start_windmill(windmill_id: str, db: Session = Depends(get_db)):
    ...
    await sim.start(windmill_id, sensor_s, location_s)
...
async def stop_windmill(windmill_id: str, db: Session = Depends(get_db)):
    """Stop the simulation loop. Idempotent — stopping a stopped windmill returns 200."""
    wm = windmill_repo.get_by_windmill_id(db, windmill_id)
    if not wm:
        raise HTTPException(status_code=404, detail="Windmill not found.")
...
def windmill_etl(windmill_id: str, db: Session = Depends(get_db)):
    """Run incremental ETL from PostgreSQL to Parquet. Synchronous — blocks until done."""
    wm = windmill_repo.get_by_windmill_id(db, windmill_id)
    if not wm:
        raise HTTPException(status_code=404, detail="Windmill not found.")
...
def get_history(windmill_id: str, scale: str = "minute", db: Session = Depends(get_db)):
    """Return Parquet-backed history for windmill_id at the requested scale."""
    wm = windmill_repo.get_by_windmill_id(db, windmill_id)
    if not wm:
        raise HTTPException(status_code=404, detail="Windmill not found.")
    if scale not in ("minute", "hour", "day", "week"):
        raise HTTPException(status_code=422, detail="scale must be minute, hour, day, or week.")
    config = windmill_repo.windmill_to_dict(wm)
    return parquet.read_history(windmill_id, scale, config)
