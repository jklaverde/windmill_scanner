...
from http import ws_registry, task_registry
...
# ---------------------------------------------------------------------------
# Async simulation loop coroutines
# ---------------------------------------------------------------------------
...
async def _sensor_loop(windmill_id: str, beat_seconds: int) -> None:
    ...
    try:
        while True:
            await asyncio.sleep(beat_seconds)  # wait one full beat before first emission

            # Fetch fresh windmill config each iteration (picks up edits after restart)
            db = SessionLocal()
            try:
            ...
            for sensor, cfg in configs.items():
                ...
                # After a range edit the stored value may be outside bounds — clamp restores it
            ...
            # Write to DB off the event loop (sync SQLAlchemy)
        ...
        pass  # last_values already preserved; task exits cleanly
                ...
                def _insert_hb() -> None:
                    ...
                    try:
                        location_repo.insert_heartbeat(
                            db2, windmill_id, now,
                            wm.lat, wm.lat_dir, wm.lon, wm.lon_dir,
                        )
                    finally:
                        db2.close()
...
# ---------------------------------------------------------------------------
# Start / stop helpers (called from async route handlers)
# ---------------------------------------------------------------------------
