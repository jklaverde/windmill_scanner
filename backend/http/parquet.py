...
def list_parquet_files(db: Session = Depends(get_db)):
    ...
    running_ids = {
        wm.windmill_id
        for wm in db.query(Windmill).filter(Windmill.is_running.is_(True)).all()
    }
...
def delete_parquet_file(windmill_id: str, db: Session = Depends(get_db)):
    """Delete the Parquet file for windmill_id. 409 if in_use (windmill is running)."""
    wm = windmill_repo.get_by_windmill_id(db, windmill_id)
    if wm and wm.is_running:
        raise HTTPException(
            status_code=409,
            detail=f"Stop windmill {windmill_id} before deleting its Parquet file.",
        )
