...
def create_farm(body: FarmCreate, db: Session = Depends(get_db)):
    ...
    if farm_repo.get_by_name(db, body.name):
        write_notification("error", f"Farm name '{body.name}' already exists — creation failed.", "farm")
...
def delete_farm(farm_id: int, db: Session = Depends(get_db)):
    ...
    if farm_repo.get_windmills(db, farm_id):
        raise HTTPException(status_code=409, detail="Remove all windmills from this farm before deleting it.")
    name = farm.name
...
async def start_farm(farm_id: int, db: Session = Depends(get_db)):
    ...
    for wm in windmills:
        try:
            ...
            await sim.start(wm.windmill_id,
                            sim.beat_to_seconds(wm.sensor_beat, wm.sensor_beat_unit),
